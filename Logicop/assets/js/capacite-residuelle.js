...
(function () {
  ...
  const FIELDS = [
    ['marque', 'Marque', false],
    ['modele', 'Modèle', false],
    ['serie', 'Numéro de série', false],
    ['cap_nominale', 'Capacité nominale (kg)', true],
    ['q_ref', 'Capacité résiduelle de référence (kg)', true],
    ['cdg_ref', 'CDG de référence (mm)', true],
    ['h_ref', 'Hauteur de référence (mm)', true],
    ['x', 'Côte x (mm)', true],
    ['s', 'Épaisseur fourches S (mm)', true],
  ];
  ...
  function renderFieldsGrid() {
    ...
    FIELDS.forEach(([key, label, required]) => {
      ...
      input.addEventListener('input', () => {
        ...
        if (state.resultQ !== null) tryRecalculate();
      });
  ...
  function tryRecalculate() {
    ...
    try {
      ...
      if (!cdgVoluRaw) { state.resultQ = null; resultCard.hidden = true; return; }
      const cdgV = parseFloat(cdgVoluRaw);
      if (isNaN(cdgV)) { state.resultQ = null; resultCard.hidden = true; return; }
  ...
  categorie.addEventListener('change', () => { if (state.resultQ !== null) tryRecalculate(); else refreshResult(); });
  fourches.addEventListener('change', () => { if (state.resultQ !== null) tryRecalculate(); else refreshResult(); });
  cdgVoulu.addEventListener('input', () => { if (state.resultQ !== null) tryRecalculate(); });
  ...
  btnCalculate.addEventListener('click', () => {
    ...
    showCalcStatus('', '');
    try {
      const REQUIRED = FIELDS.filter(([, , req]) => req).map(([key, label]) => [key, label]);
      for (const [key, label] of REQUIRED) { getFloat(key, label); }
      const qRef = getFloat('q_ref', 'Capacité résiduelle de référence');
      const cdgRef = getFloat('cdg_ref', 'CDG de référence');
      const x = getFloat('x', 'Côte x');
      const s = getFloat('s', 'Épaisseur fourches S');
