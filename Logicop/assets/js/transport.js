...
(function () {
  ...
  function showResult(label, city, lng, lat) {
    const zone = findZone(lng, lat, city);
    if (!zone) {
      searchStatus.className = 'status show warn';
      searchStatus.textContent = `${label} — cette adresse ne se situe dans aucune zone de livraison.`;
      return;
    }
    const pricePL = state.prices[zone];
    const priceTech = state.pricesTech[zone];
    resultVille.textContent = label;
    resultZone.textContent = zone;
    el('resultPrixPL').textContent = (pricePL !== undefined && pricePL !== '') ? pricePL + ' €' : 'Non configuré';
    el('resultPrixTech').textContent = (priceTech !== undefined && priceTech !== '') ? priceTech + ' €' : 'Non configuré';
  ...
  async function geocodeAndFind(query) {
    ...
    try {
      ...
      if (isPostalCode) {
        const resp = await fetch(`https://geo.api.gouv.fr/communes?codePostal=${query.trim()}&fields=nom,centre&format=json`);
        const communes = await resp.json();
        if (communes.length === 0) {
          searchStatus.className = 'status show warn';
          searchStatus.textContent = 'Aucune commune trouvée pour ce code postal.';
          return;
        }
        if (communes.length === 1) {
          const c = communes[0];
          const [lng, lat] = c.centre.coordinates;
          showResult(`${c.nom} (${query.trim()})`, c.nom, lng, lat);
          return;
        }
        searchStatus.textContent = '';
        searchStatus.className = 'status';
      ...
      if (!data.features || !data.features.length) {
        const resp2 = await fetch(`https://api-adresse.data.gouv.fr/search/?q=${encodeURIComponent(query)}&limit=1`);
        ...
        showResult(f.properties.label, f.properties.city || '', f.geometry.coordinates[0], f.geometry.coordinates[1]);
        return;
      }
  ...
  function renderZonePrices() {
    ...
    zoneNames.forEach((name) => {
      ...
      inputPL.type = 'text';
      inputPL.value = state.prices[name] || '';
      inputPL.placeholder = '0';
      inputPL.addEventListener('input', () => { state.prices[name] = inputPL.value.trim(); });
      tdPL.appendChild(inputPL);

      const tdTech = document.createElement('td');
      const inputTech = document.createElement('input');
      inputTech.type = 'text';
      inputTech.value = state.pricesTech[name] || '';
      inputTech.placeholder = '0';
      inputTech.addEventListener('input', () => { state.pricesTech[name] = inputTech.value.trim(); });
  ...
  btnSaveConfig.addEventListener('click', async () => {
    const config = { ...state.tool.config, prices: state.prices, pricesTech: state.pricesTech };
  ...
  (async function init() {
    ...
    state.pricesTech = tool.config.pricesTech || {};
