...
window.DevisEngine = (function () {
  ...
  function parseBydExcel(workbook) {
    ...
    // Nom du modèle depuis C1 (ex: "SEF35C (Comfort) Formulaire de commande" → "SEF35C (Comfort)")
    let modelName = cellStr(1, 2);
    if (modelName) {
    ...
    // Première passe : détecter les catégories et les options cochées
    const categories = []; // { startRow, endRow, aText, checkedDescs[] }
    let currentCat = null;
    ...
    for (let r = 4; r <= range.e.r + 1; r++) {
      ...
      // Nouvelle catégorie si colonne A non-vide
      if (a) {
        currentCat = { startRow: r, aText: a, checkedDescs: [] };
        categories.push(currentCat);
      }
      if (!currentCat) continue;

      // Détecter les cases cochées : G/H/I non-nul et non-zéro
      for (let pi = 0; pi < 3; pi++) {
        const pVal = cellVal(r, 6 + pi);
        if (pVal == null || typeof pVal !== 'number' || pVal === 0) continue;
        // Trouver la description : essayer C/D/E correspondant, sinon fallback
        const targetCol = 2 + pi; // G→C, H→D, I→E
        let desc = cellStr(r, targetCol).replace(/^\s+/, '');
        if (!desc) {
          // Fallback : chercher dans les autres colonnes C/D/E
          for (let fc = 2; fc <= 4; fc++) {
        ...
        if (desc && typeof desc === 'string') {
          currentCat.checkedDescs.push(desc);
        }
      }
    }
    ...
    for (const cat of categories) {
      if (cat.checkedDescs.length > 0) {
        for (const desc of cat.checkedDescs) {
          items.push({ excluded: false, desc, ht: '' });
        }
      } else {
        // Aucune case cochée → prendre le standard (colonne A)
        let stdDesc = cat.aText.split('(')[0].trim();
        if (stdDesc) {
          items.push({ excluded: false, desc: stdDesc, ht: '' });
        }
      }
    }
