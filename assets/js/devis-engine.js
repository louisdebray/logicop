...
window.DevisEngine = (function () {
  ...
  function cloneWorksheet(wb, sourceWs, newName) {
    const target = wb.addWorksheet(newName, { properties: { ...sourceWs.properties } });
    if (sourceWs.columns) {
      target.columns = sourceWs.columns.map((c) => ({ width: c && c.width }));
    }
    sourceWs.eachRow({ includeEmpty: true }, (row, rowNumber) => {
      const targetRow = target.getRow(rowNumber);
      row.eachCell({ includeEmpty: true }, (cell, colNumber) => {
        const targetCell = targetRow.getCell(colNumber);
        try { targetCell.style = JSON.parse(JSON.stringify(cell.style)); } catch { /* style non clonable, tant pis */ }
        if (cell.formula) {
    ...
    (sourceWs.model.merges || []).forEach((rangeStr) => {
      try { target.mergeCells(rangeStr); } catch { /* déjà fusionnée, sans importance */ }
    });
    return target;
  }
  ...
  return {
    ...
    getPdfLines, extractExcel, parseCesabExcel,
