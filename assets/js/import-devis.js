...
(function () {
  ...
  sourceFile.addEventListener('change', async (e) => {
    ...
    if (ext === 'pdf' && isBydSource.checked) {
      ...
      let result;
      try {
        result = await Engine.parseBydPdf(buf);
      } catch (e) {
        console.error('BYD PDF parse error:', e);
        showStatus('err', 'Erreur lors de l\'analyse du PDF : ' + (e.message || e));
        state.items = [];
        renderDestColFields();
        renderItems();
        return;
      ...
      if (!result || !result.items.length) {
        showStatus('err', "Impossible d'analyser ce fichier — vérifiez qu'il s'agit bien d'un formulaire BYD imprimé en PDF.");
        state.items = [];
      } else {
      ...
      renderDestColFields();
      renderItems();
    } else if (ext === 'pdf' && isEpSource.checked) {
      state.sourceKind = 'ep';
  ...
  btnSaveTool.addEventListener('click', async () => {
    ...
    if (state.sourceKind === 'pdf') {
      ...
      // Rien à configurer : la structure du formulaire BYD est fixe (voir Engine.parseBydPdf).
    } else {
      const cols = {};
      fields.forEach((f) => { cols[f.key] = el('srcCol_' + f.key).value; });
      config.excelConfig = {
  ...
  async function loadExistingTool() {
    ...
    if (cfg.sourceType === 'pdf' && cfg.pdfConfig) {
      ...
      renderDestColFields();
      showStatus('warn', 'Importez un formulaire BYD (PDF) ci-dessus pour tester.');
