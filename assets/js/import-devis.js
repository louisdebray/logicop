...
(function () {
  ...
  const isEpSource = el('isEpSource');
  const isBydSource = el('isBydSource');
  const cesabPanel = el('cesabPanel');
  const epPanel = el('epPanel');
  const bydPanel = el('bydPanel');
  ...
  sourceFile.addEventListener('change', async (e) => {
    ...
    if (ext === 'pdf' && isEpSource.checked) {
      ...
      if (!result || !result.items.length) {
        ...
        showStatus('ok', `${result.items.length} ligne(s) détectée(s).`);
      }
      renderDestColFields();
      renderItems();
    } else if (ext === 'pdf') {
      ...
      cesabPanel.hidden = true;
      epPanel.hidden = true;
      bydPanel.hidden = true;
      pdfTrainPanel.hidden = false;
    } else if (isBydSource.checked) {
      state.sourceKind = 'byd';
      const buf = await file.arrayBuffer();
      const pwd = prompt('Mot de passe du fichier BYD :', 'bydforklift');
      if (!pwd) { showStatus('err', 'Mot de passe requis.'); return; }
      let wb;
      try {
        if (window.decryptOfficeFile) {
          const decrypted = await window.decryptOfficeFile(buf, pwd);
          wb = XLSX.read(new Uint8Array(decrypted), { type: 'array' });
        } else {
          wb = XLSX.read(buf, { type: 'array', password: pwd });
        }
      } catch (e) {
        showStatus('err', 'Impossible de déchiffrer le fichier : ' + e.message);
        return;
      }
      pdfTrainPanel.hidden = true;
      excelOptions.hidden = true;
      ...
      if (!result || !result.items.length) {
        showStatus('err', "Impossible d'analyser ce fichier — vérifiez qu'il s'agit bien d'un formulaire BYD.");
        state.items = [];
      } else {
        state.items = result.items;
        showStatus('ok', `${result.items.length} ligne(s) détectée(s).`);
  ...
  btnSaveTool.addEventListener('click', async () => {
    ...
    } else if (state.sourceKind === 'ep') {
      // Rien à configurer : la structure du devis EP est fixe (voir Engine.parseEpPdf).
    } else if (state.sourceKind === 'byd') {
      // Rien à configurer : la structure du formulaire BYD est fixe (voir Engine.parseBydExcel).
    } else {
      const cols = {};
      fields.forEach((f) => { cols[f.key] = el('srcCol_' + f.key).value; });
      config.excelConfig = {
  ...
  async function loadExistingTool() {
    ...
    if (cfg.sourceType === 'pdf' && cfg.pdfConfig) {
    ...
    } else if (cfg.sourceType === 'ep') {
      isEpSource.checked = true;
      epPanel.hidden = false;
      FIELDS.forEach((f) => {
        const box = el('field_' + f.key);
        if (box) box.checked = (f.key === 'desc' || f.key === 'ht');
      ...
      renderDestColFields();
      showStatus('warn', 'Importez à nouveau un devis exemple EP ci-dessus pour tester.');
    } else if (cfg.sourceType === 'byd') {
      isBydSource.checked = true;
      bydPanel.hidden = false;
      FIELDS.forEach((f) => {
        const box = el('field_' + f.key);
        if (box) box.checked = (f.key === 'desc' || f.key === 'ht');
      ...
      renderDestColFields();
      showStatus('warn', 'Importez à nouveau un formulaire BYD ci-dessus pour tester.');
