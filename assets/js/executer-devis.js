...
(function () {
  ...
  sourceFile.addEventListener('change', async (e) => {
    ...
    try {
      if (ext === 'pdf' && cfg.sourceType === 'byd') {
        const buf = await file.arrayBuffer();
        state.fields = Engine.FIELDS.filter((f) => f.key === 'desc' || f.key === 'ht');
        const result = await Engine.parseBydPdf(buf);
        if (!result || !result.items.length) { showExtractStatus('err', "Impossible d'analyser ce fichier — vérifiez qu'il s'agit bien d'un formulaire BYD imprimé en PDF."); return; }
  ...
  (async function init() {
    ...
    if ((tool.config || {}).sourceType === 'byd') {
      sourceFile.accept = '.pdf';
      var bydHint = document.getElementById('bydPdfHint');
      if (bydHint) bydHint.hidden = false;
    }
