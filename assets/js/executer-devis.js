...
(function () {
  ...
  (async function init() {
    ...
    // Par défaut, le nom du fichier généré reprend le nom de l'outil (pas un nom générique
    // "Feuille de calcul.xlsx" identique pour tous les outils, source de confusion dès qu'un
    // client a accès à plusieurs outils) — sauf si l'admin a explicitement choisi un autre nom.
    const defaultFileName = tool.name.trim().normalize('NFD').replace(/[̀-ͯ]/g, '')
      .replace(/[^a-zA-Z0-9]+/g, '_').replace(/^_+|_+$/g, '') + '.xlsx';
